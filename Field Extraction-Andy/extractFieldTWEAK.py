...
def getPdfBoxes(pdf_path, img_paths, skip_corners=False):
    ...
    for i in range(len(img_paths)):
        ...
        for key in bounding_boxes[i].keys():
            for j, box in enumerate(bounding_boxes[i][key]):
                ...
                left_x = (box['box'][0] * (width) -130) * (3000/2860) #+ (3000/612*40)
                right_x = (box['box'][2]  * (width) - 130) * (3000/2860)  #+ (3000/612*40)
                top_y = ((height - box['box'][1] * (height)) - 310) * (4000/3755) #- (4000/792*40/2)
                bot_y = ((height - box['box'][3]  * (height)) - 310) * (4000/3755)  #- (4000/792*40/2)
    ...
    print (img_paths[1])
    for key in bounding_boxes[3]:
        for box in bounding_boxes[3][key]:
            if key == 'Reset':
                continue
...
async def test():
    ...
    form = adjustColor('./oafpbm/3.jpg')
    form = scaleImg(form)
    regions = fieldBounds[1]
    count  = 0
    for key in regions.keys():
        for box in regions[key]:
            # print(str(box['box']))
    ...
    img1 = adjustColor('./oafpbm/3.jpg')
    img1 = scaleImg(img1)
    future1 = await callGoogle(img1, fieldBounds[4])
    # print(future1)
